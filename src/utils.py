...
def serialiseInferenceResultsAndPutInFile(
    data: dict[tuple[mpmath.mpc, mpmath.mpf, mpmath.mpf], dict[str, mpmath.mpc]],
    filePath: str,
):
    ...
    for key in data:
        ...
        expectedInputToZeta = complexNumberToString(
            data[key][ExtractionConstants.expectedValue]
        )
        inferredInputToZeta = complexNumberToString(
            data[key][ExtractionConstants.inferredValue]
        )
        serialisedDict[inputToEta] = {
            ExtractionConstants.expectedValue: expectedInputToZeta,
            ExtractionConstants.inferredValue: inferredInputToZeta,
        }

    putInJsonRaw(serialisedDict, filePath)


def convertToSerialisableJsonAndPutInJsonFile(
    data: dict[mpmath.mpc, tuple[mpmath.mpc, mpmath.mpf, mpmath.mpf]], filePath: str
):
...
def convertToFeedablePromptAndPutInJsonFile(
    data: dict[tuple[mpmath.mpc, mpmath.mpf, mpmath.mpf], mpmath.mpc],
...
def promptCompletionFormat(key, value):
    ...
    promptDict: dict[str, str] = {
        "prompt": f"Zetavalue:{zetaValue},low:{low},high:{high}",
        "completion": output,
    }
...
# if dict1 contains any key mentioned in keysToRemove then that entry will be removed
def removeKeysFromMap1(dict1: dict, keysToRemove: set[mpmath.mpc]):
